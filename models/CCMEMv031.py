...
class CCMEMv031(object):
    ...
    def train(self, replay_buffer, batch_size=100):
        ...
        mem_contrib = torch.sum(target_Q > mem_q).cpu().item() / target_Q.shape[0]
        ...
        # Compute critic loss
        td3_loss = F.mse_loss(current_Q1, target_Q) + F.mse_loss(current_Q2, target_Q)

        # Compute critic loss
        mem_loss = F.mse_loss(current_Q1, mem_q) + F.mse_loss(current_Q2, mem_q)
        critic_loss = (1 - self.alpha) * td3_loss + self.alpha * mem_loss
